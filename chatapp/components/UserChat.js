...
const UserChat = ({ item }) => {
  ...
  const getChat = async () => {
    try {
      ...
      const userMessages = response.data.filter(
        (message) => message.messageType === 'text' || message.messageType === 'image'
      );
  ...
  return (
    <Pressable
      onPress={() => navigation.navigate('ChatMessage', { recepientId: item._id })} //id sent to chatMessage
      style={{
        flexDirection: 'row',
        alignItems: 'center',
        gap: 10,
    ...
    >
      ...
      <View>
        ...
        {lastMessage && <Text style={{ marginTop: 5, color: 'gray', fontWeight: '500' }}>{lastMessage.messageText}</Text>}
      </View>
